...
class ResNet18_CBAM(nn.Module):
    def __init__(self, num_classes=2, pretrained=True, dropout=0.5):
        ...
        # 在 layer2、layer3、layer4 后面加 CBAM
        self.cbam2 = CBAM(128)
        self.cbam3 = CBAM(256)
        self.cbam4 = CBAM(512)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset_all)):
    ...
    model = ResNet18_CBAM(num_classes=num_classes, pretrained=True, dropout=0.5)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(best_state_loss, f"{mod}/fold{fold + 1}_bestLoss.pth")
        ...
        if acc > best_val_acc:
            ...
            torch.save(best_state_acc, f"{mod}/fold{fold + 1}__bestAcc.pth")
...
for fold in range(num_folds):
    model_path = rf"D:\program\py\mpaper_decetion_10_25\ReduceLROnPlateau_result\resnet18_fold{fold+1}_bestLoss.pth"
    if not os.path.exists(model_path):
        print(f"⚠️ 未找到 {model_path}，跳过此 fold")
        continue

    model = ResNet18_CBAM(num_classes=num_classes, pretrained=False)
...
plt.rcParams['savefig.dpi'] = 800             # 默认保存分辨率
plt.rcParams['figure.dpi'] = 200              # 屏幕显示分辨率

# ------------------ 绘制混淆矩阵 ------------------
labels_names = ['良性 (0)', '恶性 (1)']
plt.figure(figsize=(7, 6))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=labels_names, yticklabels=labels_names,
            cbar=False, linewidths=0.6, square=True)
plt.title('五折集成模型的混淆矩阵', fontsize=18, pad=20)
plt.xlabel('预测标签', fontsize=14)
plt.ylabel('真实标签', fontsize=14)

# 保存为高分辨率图片（PNG + PDF）
plt.savefig('五折集成模型的混淆矩阵.png', dpi=1000, bbox_inches='tight', pad_inches=0.05)
plt.savefig('五折集成模型的混淆矩阵.pdf', bbox_inches='tight', pad_inches=0.05)
plt.show()

# ------------------ 绘制 ROC/AUC 曲线 ------------------
fpr, tpr, thresholds = roc_curve(y_true, y_prob_final)
plt.figure(figsize=(7, 6))
plt.plot(fpr, tpr, color='darkorange', lw=2.5, label=f'ROC 曲线 (AUC = {roc_auc:.3f})')
plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('假阳性率', fontsize=14)
plt.ylabel('真阳性率', fontsize=14)
plt.title('受试者工作特征曲线 (ROC)', fontsize=18)
plt.legend(loc="lower right", prop={'family': 'SimSun', 'size': 12})
plt.grid(True)
...
# 保存捕获到的控制台内容
with open(r"D:\program\py\mpaper_decetion_10_25\resnet18_CBAM_unblock\console.txt", "w", encoding="utf-8") as f:
