...
# 将所有print输出重定向到内存中，会将日志保存在txt中，但不会再控制台上输出
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset_all)):
    ...
    model = ResNet18_Only(num_classes=num_classes, pretrained=True, dropout=0.5)
    ...
    for epoch in range(num_epochs):
        ...
        if acc > best_val_acc:
            ...
            torch.save(best_state_acc, f"{mod}/fold{fold + 1}_bestAcc.pth")
...
for fold in range(num_folds):
    model_path = rf"D:\program\py\mpaper_decetion_10_25\ReduceLROnPlateau_result\onlyresnet18_fold{fold+1}_bestAcc.pth"
    if not os.path.exists(model_path):
        print(f"⚠️ 未找到 {model_path}，跳过此 fold")
        continue

    model = ResNet18_Only(num_classes=num_classes, pretrained=False)
...
# 保存捕获到的控制台内容
with open(r"D:\program\py\mpaper_decetion_10_25\only_resnet18\console.txt", "w", encoding="utf-8") as f:
